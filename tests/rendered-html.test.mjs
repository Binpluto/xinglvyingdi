...
test("supports encrypted direct Google Calendar synchronization", async () => {
  ...
  assert.match(helper, /GoogleCalendarReconnectRequired/);
  assert.match(helper, /result\.error === "invalid_grant"/);
  assert.match(helper, /DELETE FROM google_calendar_connections WHERE user_email = \?/);
  assert.match(client, /calendar_reconnect_required/);
  ...
  assert.match(gameRoute, /error instanceof GoogleCalendarReconnectRequired/);
  assert.match(gameRoute, /status: 409/);
