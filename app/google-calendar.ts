...
export class GoogleCalendarReconnectRequired extends Error {
  readonly code = "calendar_reconnect_required";

  constructor() {
    super("Google 日历授权已过期或被撤销，请重新连接后再同步");
    this.name = "GoogleCalendarReconnectRequired";
  }
}
...
async function accessToken(refreshToken: string, email: string) {
  const { clientId, clientSecret } = config();
  const response = await fetch("https://oauth2.googleapis.com/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
  ...
  if (!response.ok || !result.access_token) {
    console.error("Google token refresh failed", { status: response.status, error: result.error || "unknown" });
    if (result.error === "invalid_grant") {
      await runtime().DB.prepare("DELETE FROM google_calendar_connections WHERE user_email = ?")
        .bind(email).run();
      throw new GoogleCalendarReconnectRequired();
    }
    if (result.error === "invalid_client" || result.error === "unauthorized_client") {
      throw new Error("Google 日历服务配置异常，请稍后重试");
    }
    throw new Error(result.error_description && result.error_description !== "Bad Request"
      ? result.error_description
      : "Google 日历授权暂时不可用，请重新连接后再试");
  }
...
export async function syncGoogleCalendar(email: string, requestedSyncDate?: string) {
  ...
  const token = await accessToken(await decryptRefreshToken(connection.refresh_token, email), email);
