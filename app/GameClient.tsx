...
function QuestBoard({ data, done: _allDone, act, compact = false }: { data: GameData; done: number; act: (p: Record<string, unknown>, s: string) => Promise<boolean>; compact?: boolean }) {
  ...
  const sourceLabel = (source: string) => source === "system-daily-easy" ? "系统 · 简单" : source === "system-daily-medium" ? "系统 · 普通" : source === "system-daily-hard" ? "系统 · 困难" : source.startsWith("google") ? "Google" : source === "outlook" ? "Outlook" : source === "icloud" ? "iCloud" : source === "ics" ? "ICS" : "";
